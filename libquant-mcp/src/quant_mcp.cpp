...
static std::string buildResult(const std::string& id, const std::string& resultJson)
{
    ...
    // ID might be a number (unquoted), string (needsquoting), or "null" literal
    if (id.empty() || id == "null") {
        resp.add("id", "null");
    } else if (id[0] == '"' || (id[0] >= '0' && id[0] <= '9')) {
        // Already quoted string or a number, use as-is
        resp.add("id", id);
    } else {
        // Plain string, needs quoting
        resp.add("id", jStr(id));
    }
    resp.add("result", resultJson);
    return resp.str();
}
...
static std::string buildError(const std::string& id, int code, const std::string& msg)
{
    ...
    JObj resp;
    resp.add("jsonrpc", jStr("2.0"));
    // ID might be a number (unquoted), string (needs quoting), or "null" literal
    if (id.empty() || id == "null") {
        resp.add("id", "null");
    } else if (id[0] == '"' || (id[0] >= '0' && id[0] <= '9')) {
        // Already quoted string or a number, use as-is
